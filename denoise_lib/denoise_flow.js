...
import {hanning_window, time2freqP_1frame, freq2time_1frame, sumVectors, addVector, mulVectorScala} from './denoise_algorithm.js'
...
export class DenoiseFlow{
    constructor(frameSize = 8192, frameShift = 2048, volEstBufferSize = 10, buffer1Size=32, buffer2Size=32, sort_low = 5, sort_high = 16){
        ...
        this.volEstBuffer = new CyclicBuffer(volEstBufferSize,()=>{return time2freqP_1frame(Array.from({length: this.frameSize},()=>{return 0;}),0)})
        this.volEst = 0;
    ...
    process(input, output, param){
        ...
        if (this.inputBuffer.buffer_write_p_min_read_p >= this.frameShift){
            ...
            // volume estimation
            this.volEstBuffer.swap(frame_fft_info);
            ...
            // cancel noise
            let freqPower = addVector(frame_fft_info.freqPow, noisePower);
            // rebuild signal
            ...
            this.outputBuffer.add_array(frame_time_rebuild, false);
            this.outputBuffer.increase_write_pointer(this.frameShift);
        }
        // retrieve processed data
        output = this.outputBuffer.read_array(output.length);
